...
from slurm_engine_pretrain import train_one_epoch
...
def main(args):
    ...
    data_loader_train = torch.utils.data.DataLoader(
        dataset_train, sampler=sampler_train, batch_size=args.batch_size, num_workers=args.num_workers, pin_memory=True, drop_last=False
    )
    # why no validation?
    ...
    if (args.invisible_lr_scale is not None) and (not args.same_encoder):
        visible_params = optim_factory.add_weight_decay(model_without_ddp.visible_mae, args.weight_decay)
        ...
        if abs(args.invisible_lr_scale) <= 1e-8:
            ...
            for param in model_without_ddp.invisible_mae.parameters():
                param.requires_grad = False
        else:
            invisible_params = optim_factory.add_weight_decay(model_without_ddp.invisible_mae, args.weight_decay)
